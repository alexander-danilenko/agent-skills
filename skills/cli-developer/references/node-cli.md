...
program.name("mycli").description("My awesome CLI tool").version(version);
...
await Promise.all([processApi(bar1), processWeb(bar2)]);
...
describe("mycli", () => {
  ...
  it("handles invalid command", async () => {
    await expect(execaCommand("node bin/cli.js invalid")).rejects.toThrow();
  });
});
```
